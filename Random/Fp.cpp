...
void FindRoutes(string start, string end, int T[30], string cit[30]){
	...
	if(t1==true){
		cout<<"\n\n\n!!!The given destination doesn't exist!!!!\n\n";
		main();
	}
	for(int i=0; i<30; i++){
		string ne = cit[i];
	...
	if(e>s){
		...
		cout<<"\n\n!!!!Select the destinations in order: \n\n\n";
		main();
	}
	
	cout<<"\n\nThe Total distance from "<<start<<" To "<<end<<" is "<<tt<<"\n\n";
	
	cout<<"\n\nDo you want to select again? yes(1)/no(0): ";
	int x;
	cin>>x;
	if(x==1)
		main();
...
//		cout<<"T["<<row<<"]["<<col<<"]---"<<min<<endl;
	...
	Operator(R);
	cout<<"\n\nTotal Minimum Distance is:- "<<total_sum<<"\n\n";
	cout<<"\n\nYour Routes are:\n\n";
	show(R, no_of_edges);
	cout<<"\n\n";
...
int main(){
    ...
    string cities[60]= {"Islamabad","Karachi","Hyderabad","Sukkur","Mirpur_Khas ","Larkana",
 "Jacobabad","Shikarpur","Dadu","Khairpur","Naushero_Feroze ","Sahiwal","DG_Khan","Muzaffargarh","Tando_Allahyar "
